...
public class PamAxisFX {
	 ...
	 */
	private DoubleProperty minValProperty=new SimpleDoubleProperty(0); ;
	
	/**
	 * Maximum value property. 
	 */
	private DoubleProperty maxValProperty=new SimpleDoubleProperty(1);
...
//	private String longestString;
	...
	private ArrayList<Point2D> getLinearPoint2Ds(boolean extraOne) {
		...
		if (forceFirstVal) {
			axisValue = forcedFirstVal;
			x = x1 + axisScale * (axisValue-minValProperty.getValue()) * xDirection;
			y = y1 + axisScale * (axisValue-minValProperty.getValue()) * yDirection;
		}
...
//			DrawTickAndLabel(g2d, x, y, Math.pow(10, axisValue));
	...
	protected String formatValue(double val) {
		if (format == null) {
			if (val==0) return "0";
			return String.valueOf(val);///PamCalendar.formatDuration((long) (val*1000.));  //TODO
		}
		if (integerFormat) {
...
//			g.setStroke(new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 1, dashes, 0));
		...
		for (int i = 0; i < axisPoint2Ds.size(); i++) {
			...
			gx1 = (int) (pt.getX() - xExtent - x1 + insets.getLeft());
			gy1 = (int) (pt.getY() - yExtent - y2 + insets.getTop()); 
			gx2 = (int) (pt.getX() + xExtent - x1 + insets.getLeft());
			gy2 = (int) (pt.getY() + yExtent - y2 + insets.getTop());
