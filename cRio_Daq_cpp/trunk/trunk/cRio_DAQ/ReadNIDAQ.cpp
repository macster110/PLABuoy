...
const int bufferSize=5*1024*1024; //5MB buffer
/*Number of samples to Aquire from FIFO on each loop iteration*/
const unsigned int Number_Acquire = 1024;
...
void read_Data_Buffer(int channels){
	...
	while (daq_go && NiFpga_IsNotError(status_DAQ))
	{
		...
		if (is_Sound_File_Error(error)) NiFpga_MergeStatus(&status_DAQ, NiFpga_Status_External_Storage);
